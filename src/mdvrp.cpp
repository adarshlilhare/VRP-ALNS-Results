...
struct VRPData {
    ...
    bool matrix_ready = false;
        ...
        double raw_d = sqrt(dx * dx + dy * dy);
                    double d = floor(raw_d * 100.0) / 100.0;
        if (use_truncated_math) d = floor(d * 10.0) / 10.0;
        return d;
    }
...
VRPData load_vrp(string filename) {
    ...
    while (getline(file, line)) {
        ...
        if (reading_coords) {
            ...
            if (ss >> id >> x >> y) {
                if (ss >> d >> r >> du >> s) {}
                data.nodes.push_back({id, x, y, (long long)d, (long long)r, (long long)du, (long long)s});
            }
        }
    }
...
vector<vector<int>> solomon_i1(const VRPData& data, int sort_mode) {
    ...
    if (sort_mode == 0) {
        // Sweep Algorithm: sort by polar angle from depot
        double cx = data.nodes[0].x;
        double cy = data.nodes[0].y;
        sort(candidates.begin(), candidates.end(), [&](int a, int b) {
            double angleA = atan2(data.nodes[a].y - cy, data.nodes[a].x - cx);
            double angleB = atan2(data.nodes[b].y - cy, data.nodes[b].x - cx);
            return angleA < angleB;
        });
    } else if (sort_mode == 1) {
        sort(candidates.begin(), candidates.end(), [&](int a, int b) { return data.nodes[a].due < data.nodes[b].due; });
    } else if (sort_mode == 2) {
        sort(candidates.begin(), candidates.end(), [&](int a, int b) { return data.nodes[a].demand > data.nodes[b].demand; });
    } else if (sort_mode == 3) {
        sort(candidates.begin(), candidates.end(), [&](int a, int b) { return data.dist(0, a) > data.dist(0, b); });
    } else if (sort_mode == 4) {
        sort(candidates.begin(), candidates.end(), [&](int a, int b) { 
            return (data.nodes[a].due - data.nodes[a].ready) < (data.nodes[b].due - data.nodes[b].ready); 
        });
    }
    
    // Clarke & Wright Savings Initialization (Mode 5)
    if (sort_mode == 5) {
        vector<vector<int>> cw_routes;
        vector<int> node_to_route(data.nodes.size(), -1);
        for (int i = 1; i < data.nodes.size(); ++i) {
            cw_routes.push_back({i});
            node_to_route[i] = cw_routes.size() - 1;
        }
        ...
        for (int i = 1; i < data.nodes.size(); ++i) {
            for (int j = i + 1; j < data.nodes.size(); ++j) {
                double s = data.dist(i, 0) + data.dist(0, j) - data.dist(i, j);
                if (s > 0) savings.push_back({i, j, s});
            }
        }
        sort(savings.begin(), savings.end(), [](const Saving& a, const Saving& b) { return a.s > b.s; });
        
        for (const auto& sav : savings) {
            int r1 = node_to_route[sav.i];
            int r2 = node_to_route[sav.j];
            if (r1 == r2 || r1 == -1 || r2 == -1) continue;
            
            // Check if sav.i is at the end of r1, and sav.j is at the start of r2
            if (cw_routes[r1].back() != sav.i && cw_routes[r1].front() != sav.i) continue;
            if (cw_routes[r2].back() != sav.j && cw_routes[r2].front() != sav.j) continue;
            
            // Align routes
            vector<int> merged = cw_routes[r1];
            if (cw_routes[r1].front() == sav.i) reverse(merged.begin(), merged.end());
            
            vector<int> to_add = cw_routes[r2];
            if (cw_routes[r2].back() == sav.j) reverse(to_add.begin(), to_add.end());
            
            long long load = 0;
            for (int n : merged) load += data.nodes[n].demand;
            for (int n : to_add) load += data.nodes[n].demand;
            
            if (load > data.capacity) continue;
            
            vector<int> test_merge = merged;
            test_merge.insert(test_merge.end(), to_add.begin(), to_add.end());
            
            if (update_route_times(test_merge, data)) {
                cw_routes[r1] = test_merge;
                cw_routes[r2].clear();
                for (int n : test_merge) node_to_route[n] = r1;
            }
        }
        
        vector<vector<int>> final_routes;
        for (const auto& r : cw_routes) if (!r.empty()) final_routes.push_back(r);
        return final_routes;
    }
                ...
                for (size_t pos = 0; pos <= current_route.size(); ++pos) {
                    ...
                    if (update_route_times(cand_route, data)) {
                        ...
                        if (data.capacity >= 500) {
                            ...
                            if (sort_mode == 1) cost = dist_cost + (time_shift / 10.0);
                            else if (sort_mode == 2) cost = (dist_cost * 10.0) + time_shift;
                            else if (sort_mode == 3) cost = time_shift;
                            else if (sort_mode == 4) cost = dist_cost + time_shift;
                        }
                        
                        if (cost < best_cost) { best_cost = cost; best_u = u; best_pos = pos; }
                    }
                }
                if (sort_mode == 0 && best_u != -1 && data.capacity < 500) break; // Strict Sweep ordering only for narrow windows
            }
            if (best_u != -1) {
...
int main(int argc, char* argv[]) {
    ...
    for (int mode = 0; mode <= 5; ++mode) {
        vector<vector<int>> routes = solomon_i1(data, mode);
        ...
        if (v_count < best_vehicles || (v_count == best_vehicles && fit < best_total_cost)) {
    ...
    ofstream log_file("mdvrp_convergence.csv");
    log_file << "Time,Cost\n";
    log_file.close();
    ...
    #pragma omp parallel
    {
        ...
        while (true) {
            ...
            if (omp_get_thread_num() == 0 && (iter_count % 100 == 0)) {
                ofstream log_file_app("mdvrp_convergence.csv", ios::app);
    ...
    for (size_t i = 0; i < best_routes.size(); ++i) {
        // Filter out depot/padding zeros
        vector<int> clean_route;
        for (int node : best_routes[i]) {
            if (node != 0) clean_route.push_back(node);
        }
        if (clean_route.empty()) continue; // Skip empty routes
        cout << "ROUTE " << i << ": ";
        for (int node : clean_route) cout << node << " ";
        cout << endl;
    }
